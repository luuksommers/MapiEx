...
{ 
	...
	if(!CMAPIEx::Init(TRUE, TRUE) || !mapi.Login(NULL, TRUE) || !mapi.OpenMessageStore() || !mapi.Notify(OnNewMessage, NULL, fnevNewMail)) 
	{
...
{ 
	if(argc>1)
	{
		...
		if(hManager) 
		{
			...
			{
				if (InstallService(hManager)) PRINTF(_T("Installed %s successfully.\n"), SERVICE_NAME);
				else PRINTF(_T("Failed to install %s.\n"), SERVICE_NAME);
			} else if(!_tcscmp(argv[1], _T("-u"))) 
			{
				if (hService && DeleteService(hService)) PRINTF(_T("Uninstalled %s successfully.\n"), SERVICE_NAME);
				else PRINTF(_T("Failed to uninstall %s.\n"), SERVICE_NAME);
			} else {
				PRINTF(_T("Usage:\n"));
