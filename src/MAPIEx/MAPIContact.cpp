...
BOOL CMAPIContact::Create(CMAPIEx* pMAPI, CMAPIFolder* pFolder)
{
	...
	prop.Value.LPSZ=_T("IPM.Contact");
...
BOOL CMAPIContact::GetNotes(CString& strNotes, BOOL bRTF)
{
	strNotes=_T("");
#ifdef _WIN32_WCE
	return GetPropertyString(PR_BODY, strNotes);
#else
	IStream* pStream;
	...
	if(bRTF) 
	{
		if(Contact()->OpenProperty(PR_RTF_COMPRESSED, &IID_IStream,STGM_READ, 0, (LPUNKNOWN*)&pStream)!=S_OK) return FALSE;
	...
	{
		if(Contact()->OpenProperty(PR_BODY, &IID_IStream,STGM_READ, NULL, (LPUNKNOWN*)&pStream)!=S_OK) return FALSE;
...
BOOL CMAPIContact::SetPicture(LPCTSTR szPath)
{
	...
	SetOutlookProperty(CMAPIContact::OUTLOOK_DATA1, CMAPIContact::OUTLOOK_PICTURE_FLAG, bPicture);
